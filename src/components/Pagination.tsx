...
const Pagination: React.FC<Props> = ({
  ...
  const getPages = () => {
    if (maxPages < 8) {
      const arr = [];
      for (let i = 0; i < maxPages; i++) arr.push(i);
      return arr;
    }
    ...
    if (currentPage > 2) {
      if (currentPage < maxPages - 3) {
        arr.push('Left', currentPage - 1, currentPage, currentPage + 1, 'Right', maxPages - 1);
        return arr;
      } else {
        arr.push('Left', maxPages - 4, maxPages - 3, maxPages - 2, maxPages - 1);
        return arr;
      }
    }
      ...
      <StyledPagination>
        <Arrow onClick={previousPage} isDisabled={!currentPage}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.99998 13.28L5.65331 8.93333C5.13998 8.42 5.13998 7.58 5.65331 7.06667L9.99998 2.72" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </Arrow>
        <Pages>
          {getPages().map((el) => {
            if (el === 'Left') return (
              <PageNumber key="Left" onClick={() => changePage(1)}>
                ...
              </PageNumber>
            );
            if (el === 'Right') return (
              <PageNumber key="Right" onClick={() => changePage(maxPages - 2)}>
                ...
              </PageNumber>
            );
            return (
              <PageNumber key={el} isActive={currentPage === +el} onClick={() => changePage(+el)}>
                {+el + 1}
              </PageNumber>
            );
          })}
        ...
        <Arrow onClick={nextPage} isDisabled={currentPage + 1 === maxPages}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M5.93994 13.28L10.2866 8.93333C10.7999 8.42 10.7999 7.58 10.2866 7.06667L5.93994 2.72" strokeWidth="1.5" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </Arrow>
      </StyledPagination>
      <Info>
